...
public class Solution {
    ...
    */
    ...
    public boolean isSuccessful(GameModel model){
        ...
        for(int i = 0; i < height ; i++){
            for(int j = 0; j < width; j++) {
                if(!oddNeighborhoodWithModel(i,j,model)){
                    return false;
                }
            }
        }
    ...
    */
    ...
    public boolean stillPossible(boolean nextValue, GameModel model) {
        ...
        if((i > 0) && (!oddNeighborhoodWithModel(i-1,j,model))){
            possible = false;
        }
        if(possible && (i == (height-1))) {
            if((j > 0) && (!oddNeighborhoodWithModel(i,j-1,model))){
                possible = false;
            }
            if(possible && (j == (width-1))&& (!oddNeighborhoodWithModel(i,j,model))){
                possible = false;            
            }
        }
        board[i][j] = before;
...
*/
        ...
        while(currentIndex < height*width) {
            if(i < height - 1 ) {
                setNext(!oddNeighborhoodWithModel(i-1,j,model));
                i = currentIndex/width;
                j = currentIndex%width;
            } else { //last raw
                if(j == 0){
                   ...
                   if((height > 1) && oddNeighborhoodWithModel(i-1,j,model) != oddNeighborhoodWithModel(i,j-1,model)){
                     return false;
                   }
                   setNext(!oddNeighborhoodWithModel(i,j-1,model));
                } 
                i = currentIndex/width;
                j = currentIndex%width;
            }
        }
        if(!oddNeighborhoodWithModel(height-1,width-1,model)){
            return false;
        }
        // here we should return true because we could
        // successfully finish the board. However, as a
    ...
    private boolean oddNeighborhoodWithModel(int i, int j, GameModel model) {
